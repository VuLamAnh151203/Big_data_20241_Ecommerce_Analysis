...
#     cluster_data = user_features.filter(user_features.cluster == cluster_id)
#     unique_users = cluster_data.count()
#     avg_activity = cluster_data.agg(F.mean("activity_frequency")).collect()[0][0]
#     avg_spend = cluster_data.agg(F.mean("average_spend")).collect()[0][0]
#     avg_unique_products = cluster_data.agg(F.mean("unique_products")).collect()[0][0]
#     avg_unique_brands = cluster_data.agg(F.mean("unique_brands")).collect()[0][0]

#     print(f"\nCluster {cluster_id} - Behavioral Characteristics:")
